...
const SlotBooking = () => {
  ...
  useEffect(() => {
    ...
    fetch('https://backend-1-80uy.onrender.com/api/slots')
      .then((res) => {
  ...
  const handleBooking = async (slotId) => {
    try {
      const response = await fetch(`https://backend-1-80uy.onrender.com/api/book-slot/${slotId}`, {
        method: 'POST',
        headers: {
  ...
  const handleUnblock = async (slotId) => {
    try {
      const response = await fetch(`https://backend-1-80uy.onrender.com/api/unblock-slot/${slotId}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
