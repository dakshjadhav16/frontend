...
export default function PostCreate() {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            try {
                const response = await fetch(`https://backend-1-80uy.onrender.com/user/${loggedData.loggedUser.userid}/posts`, {
                    method: 'GET',
                    headers: {
    ...
    const handleDeletePost = async (postId) => {
        ...
        if (confirmDelete) {
            try {
                const response = await fetch(`https://backend-1-80uy.onrender.com/posts/${postId}`, {
                    method: 'DELETE',
                    headers: {
                        'Authorization': `Bearer ${loggedData.loggedUser.token}`,
                    },
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('https://backend-1-80uy.onrender.com/posts', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${loggedData.loggedUser.token}`,
               },
