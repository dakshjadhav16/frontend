...
const Profile = () => {
  ...
  useEffect(() => {
    ...
    fetch(`https://backend-1-80uy.onrender.com/userinfo/${loggedData.loggedUser.userid}`, {
      method: 'GET',
      headers: {
  ...
  useEffect(() => {
    if (loggedData.loggedUser && loggedData.loggedUser.userid) {
      fetch(`https://backend-1-80uy.onrender.com/upload_profile_photo/${loggedData.loggedUser.userid}`, {
        method: 'GET',
        headers: {
          "Authorization": `Bearer ${loggedData.loggedUser.token}`,
        }
  ...
  useEffect(() => {
    fetch(`https://backend-1-80uy.onrender.com/track/${loggedData.loggedUser.userid}/${date.getMonth()+1}-${date.getDate()}-${date.getFullYear()}`, {
      method: "GET",
      headers: {
        "Authorization": `Bearer ${loggedData.loggedUser.token}`
      }
  ...
  /*useEffect(() => {
    if (userPhysics.height && userPhysics.weight) {
      const heightInMeters = userPhysics.height / 100;
      const bmiValue = (userPhysics.weight / (heightInMeters ** 2)).toFixed(2);
      setPhysicalInfo((prevInfo) => ({
        ...prevInfo,
        bmi: bmiValue,
      }));
    }
  }, [userPhysics.height, userPhysics.weight]); 
  ...
  useEffect(() => {
    if (loggedData.loggedUser && loggedData.loggedUser.userid) {
      ...
      fetch(`https://backend-1-80uy.onrender.com/water_intake/${loggedData.loggedUser.userid}/${currentDate}`, {
        method: 'GET',
        headers: {
          "Authorization": `Bearer ${loggedData.loggedUser.token}`,
        }
  ...
  const handleAddGlass = () => {
    ...
    fetch('https://backend-1-80uy.onrender.com/water_intake', {
      method: 'POST',
      headers: {
        "Authorization": `Bearer ${loggedData.loggedUser.token}`,
  ...
  const handlePhotoUpload = (e) => {
    ...
    if (file) {
      ...
      fetch('https://backend-1-80uy.onrender.com/upload_profile_photo', {
        method: 'POST',
        headers: {
          "Authorization": `Bearer ${loggedData.loggedUser.token}`,
        },
...
/* const handleSubmit = (e) => {
    ...
    fetch("https://backend-1-80uy.onrender.com/physical_info", {
      method: "POST",
      headers: {
        "Authorization": `Bearer ${loggedData.loggedUser.token}`,
        "Content-Type": "application/json"
